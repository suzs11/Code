...
validCriteria = {'firstBelow', 'localMin', 'firstDrop'};
...
for c=1:ncol
    ...
    elseif strcmp(criterion, 'firstDrop')
        lags(c) = findFirstDrop(auto_mi(:, c), maxLag, plotType, threshold);
    end
end
...
% Yong's code
function lag = findFirstDrop(ami, maxLag, plotType, threshold)
    % This function finds the position at which the mean of ami changes significantly
    
    % derivative of ami
    dami = -diff(ami);
    % find the position at which the rate of ami's derivative changes significantly
    idx1 = findchangepts(dami); 
    % get the position of the first occurance of dami that is less than percent of dami(1)
    percent = 0.5;
    c1 = dami<=percent*dami(1);
    c2 = dami>=0;
    c = c1 & c2;
    idx2 = find(c, 1, 'first');
    
    % if idx2 is empty, relax the condition
    factor = 1.1;
    while isempty(idx2)        
        c1 = dami<=factor*percent*dami(1);
        c = c1 & c2;
        idx2 = find(c, 1, 'first');
        factor = 1.1* factor;
    end
    ...
    if strcmp(plotType, 'all')
        disp(['idx1=',num2str(idx1), ' idx2=', num2str(idx2)]);
        figure
        plot(dami,'r');
        xlabel('Time Lag')
        ylabel('Differential Mutual Inforamtion')
        xticks(1:length(dami));
        plotAllMI(ami, threshold);
        %pause
    ...
    % A value of the index idx = 1 corresponds to lag = 0, so 1 is subtracted from the index to get the lag.
    %lag = idx(end) - 1;     
    % If we are using diff, then no need to subtract one from idx
    lag = idx(end);
