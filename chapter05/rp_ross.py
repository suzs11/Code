...
if __name__=="__main__":
    ...
    fig2 = plt.figure(num=None, figsize=((15,4)), dpi= 100)
    plt.subplot(1,3,1)
    plt.plot(t,traj[:,0])
    plt.xlim(0, 100)
    plt.xlabel("$\it{t}$", fontsize=11)
    plt.ylabel('$\it{x(t)}$', fontsize=11)
    plt.title('A', fontsize=16, loc='left')
    #plt.axes([0.1, 0.1, 0.15, 0.25])
    X = traj[1000:,0]
    dimension, delay, threshold, norm = 3, 6, 15, "manhattan"
    ...
    dimension, delay, threshold, norm = 3, 5, 7, "manhattan"
    #distance_matrix = distance_matrix(X, dimension, delay, norm)
    #recurrence_matrix = recurrence_matrix(X, dimension, delay, threshold, norm)
    #LIN = len(distance_matrix[:,0])
    ...
    dimension, delay, threshold, norm = 3, 5, 0.7, "euclidean"
    #LIN = len(distance_matrix[:,0])
    ...
    dimension, delay, threshold, norm = 3, 5, 0.7, "supremum"
    #distance_matrix = distance_matrix(X, dimension, delay, norm)
    #recurrence_matrix = recurrence_matrix(X, dimension, delay, threshold, norm)
    #LIN = len(distance_matrix[:,0])
